...
// Look in user-created graphs (localStorage or Matrix) first, then in the
// bundled fixtures. Same shape either way; the page doesn't care which
// source the graph came from.
export const load: PageLoad = async ({ params }) => {
	const userGraph = await getUserGraph(params.id);
