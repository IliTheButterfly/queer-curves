...
- `OccurrenceGraph` (counters with units/steps/quick-add presets/optional targets, plus `occurrences` — discrete timestamped amounts). Counting, not positioning: see `data_model.md` §4A. All roll-up logic is pure and lives in `graphs/occurrence/aggregate.ts` (bucketing, targets, streaks) — it's local-time aware with a configurable `day_start_hour`, and it's the only place that knows how a day is defined, so put new derived figures there rather than in components.
